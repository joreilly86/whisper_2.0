...
def parse_markdown_to_notion_blocks(content):
    ...
    while i < len(lines):
        ...
        # Headings
        if line.startswith('#'):
            heading_level = min(len(line) - len(line.lstrip('#')), 3)  # Notion supports h1, h2, h3
        ...
        # Regular paragraph
            ...
            if len(paragraph_text) > config.NOTION_MAX_CHUNK_SIZE:
                chunks = [paragraph_text[j:j + config.NOTION_MAX_CHUNK_SIZE] 
                ...
                for chunk in chunks:
                    blocks.append({
                        ...
                        "type": "paragraph",
                        "paragraph": {
                            "rich_text": parse_rich_text(chunk)
                        }
                    })
            else:
                blocks.append({
    ...
    for part in parts:
        ...
        if part.startswith('**') and part.endswith('**'):
            ...
            for italic_part in italic_parts:
                if not italic_part:
                    continue
                if (italic_part.startswith('*') and italic_part.endswith('*') and 
                    not italic_part.startswith('**')):
                    # Italic text
                ...
                elif (italic_part.startswith('_') and italic_part.endswith('_') and 
                      not italic_part.startswith('__')):
                    # Italic text
                    italic_text = italic_part[1:-1]
                    rich_text.append({
                        "type": "text",
